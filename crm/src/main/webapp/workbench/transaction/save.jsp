...
<script type="text/javascript" src="jquery/bs_typeahead/bootstrap3-typeahead.min.js"></script>
	<script>
		...
		/*
		关于阶段与可能性：
		是一一对应的关系，一个阶段对应一个可能性
		我们现阶段可以将阶段和可能性想象成一个键值对的关系
		...
		1、数据量不是很大
		2、键值对应的关系
		如果同时满足这两种结论，我们将数据保存在数据库表中是没有什么意义的
		如果遇到这种情况，我们需要用到properties属性文件中
		这个文件表示的是阶段和键值对之间的对应关系，我们通过stage以及对应关系，来取得可能性这个值
		这种需求在交易模块中需要大量的使用到，所以我们就需要将该文件解析在服务器中缓存
		application.setAttribute(.properties文件的内容)
         以上我们已经将json处理了接下来的取可能性
	...
	$(function () {
		...
		$("#create-stage").change(function () {
			...
			//alert(stage)
			/*
			我们是用json.key的形式不能取得value
			因为今天的stage是一个可变的变量，如果是这样的可以
			就不能以传统的json.key取值
			我们需要的取值方式是json[key];
			*/
			...
			目标：填写可能性
			阶段有了stage
			阶段和可能性之间的对应关系也有pMap，在服务器的缓存中呢
			pMap是java中的键值对关系，但是咱们现在在js中，怎么办？？
			我们首先将pMap转换为js的键值对关系。js中的键值对关系，只有一种，那就是
			json
			将pMap转换为json对象
			 */
	...
	<form action="workbench/transaction/save.do" id="tranForm" method="post" class="form-horizontal" role="form" style="position: relative; top: -30px;">
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" value="54013720ba3e47ad8281a9e75bb52f06" name="activityId">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" value="8d11a6df78344dddb7cabfaa4bb6beb7" name="contactsId">
			</div>
		</div>
