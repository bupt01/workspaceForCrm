...
<script type="text/javascript">
	...
	function changeIcon(stage,index1) {
		...
		//当前阶段的可能性,因为已经局部刷新了 不能使用request域对象的
		var currentPossibility=$("#possibility").html();
		//当前阶段的下标
		var index=index1;
		//前面正常化阶段和后面丢失阶段的分界点下标,java的变量转换成为js变量，必须是八大基本类型和
		//String类型
		var point=<%=point%>
		// alert("当前阶段"+currentStage);
		// alert("当前阶段可能性"+currentPossibility);
		// alert("当前阶段的下标"+index);
		// alert("当前正常阶段和后面丢失的阶段的分界点的下标"+point)
		//如果当前阶段的可能性为0,前七个一定是黑圈,后两个一个是红叉，一个是黑×
		...
		//如果当前阶段的可能性不为0，前面七个绿圈标记，黑圈，后两个一定是黑×
		}else{
			//遍历前七个
			for (var i=0;i<point;i++){
				//如果是当前阶段
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		<%
			...
			//先来判断一下当前阶段
			//如果当前阶段的可能行为0,前七个一定是黑圈，后两个不一定
			if ("0".equals(currentPossibility)){
				for (int i=0;i<dvList.size();i++){
					//取得每一个遍历出来的阶段，根据每一个遍历出来的阶段取其可能性
					Dicvalue dv=dvList.get(i);
					String listStage=dv.getValue();
					String listPosssibility=pMap.get(listStage);
					//如果遍历出来的阶段可能性为0，说明是后两个
					if ("0".equals(listPosssibility)){
						//如果当前阶段
						if (listStage.equals(currentPossibility)){
						//红叉-------------
				%>
		...
		<%
           ...
           //如果当前阶段的可能性不为0，前七个可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
			}else{
				//准备当前阶段的分界点，下标
				int index=0;
				for (int i=0;i<dvList.size();i++){
		...
		<%
						...
						//如果遍历出来的可能性不为0,说明是前七个阶段绿圈，绿色标记，黑圈
					}else{
						//如果是当前阶段
						if (i==index){
							//绿色标记
